...
export default function LeafletMap() {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      // Add prefecture markers (全47都道府県)
      PREFECTURE_GEO.forEach((pref) => {
        const price = pref.avgPrice ?? 0;
        if (!price) return;
      ...
      map.on("popupopen", (e: L.PopupEvent) => {
        ...
        if (!container) return;
        const btn = container.querySelector(".pref-popup-btn") as HTMLElement;
        if (btn) {
          btn.onclick = () => {
  ...
  return (
    <div className="relative w-full h-full rounded-2xl overflow-hidden">
      ...
      <div className="absolute bottom-4 left-4 z-[1000] bg-slate-900/90 backdrop-blur-sm rounded-xl px-4 py-3 text-xs">
        <div className="text-slate-400 font-medium mb-2">全国 平均取引価格（万円）</div>
